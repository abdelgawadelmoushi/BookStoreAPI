...
(function () {
    ...
    var getLinks = function (editor) {
      ...
      return new global$1(function (callback) {
        ...
        return optItems.bind(ListOptions.sanitizeWith(extrauthor)).map(function (items) {
          if (items.length > 0) {
            var noneItem = [{
                text: 'None',
                value: ''
    ...
    var getRels = function (editor, initialTarget) {
      ...
      if (list.length > 0) {
        ...
        var safeRelExtrauthor = function (item) {
          return applyRelTargetRules(ListOptions.getValue(item), isTargetBlank_1);
        };
        var sanitizer = enforceSafe ? ListOptions.sanitizeWith(safeRelExtrauthor) : ListOptions.sanitize;
